...
export class PaymentComponent {
    ...
    this.orderService.getOrderById(orderId).subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        ...
        if (error.status === 404) {
          ...
          this.orderError = 'Access denied to this order. You might not be the owner or an administrator.';
  ...
  processPayment(): void {
    ...
    if (this.selectedPaymentMethod === 'RAZORPAY') {
      ...
      this.paymentErrorMessage = 'Only Razorpay is supported for online payments currently. Please select Razorpay.';
      this.processingPayment = false;
    }
  }
  ...
  private initiateRazorpayPayment(): void {
    ...
    this.paymentService.createRazorpayOrder(this.order.totalAmount, 'INR', this.order.id.toString()).subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        this.paymentErrorMessage = 'Failed to create Razorpay order. Please try again.';
        this.processingPayment = false;
  ...
  private openRazorpayCheckout(razorpayOrder: RazorpayOrderResponseDTO): void {
    if (typeof Razorpay === 'undefined') {
      this.paymentErrorMessage = 'Razorpay SDK not loaded. Please ensure your internet connection is stable or try again later.';
      this.processingPayment = false;
      console.error('PaymentComponent: Razorpay object is undefined. Ensure checkout.js is loaded in index.html');
      return;
    }
    ...
    rzp.on('payment.failed', (response: any) => {
      this.paymentErrorMessage = `Payment failed: ${response.error.description || 'Unknown error'}. Please try again.`;
      this.processingPayment = false; 
  ...
  private captureRazorpayPayment(response: any): void {
    if (!this.orderId || !this.order || !this.order.totalAmount) {
      this.paymentErrorMessage = 'Internal order details missing for payment capture. Please reload the page.';
      this.processingPayment = false;
      return;
    }

    this.paymentService.captureRazorpayPayment({
      ...
      error: (error: HttpErrorResponse) => {
        this.paymentErrorMessage = 'Payment verification failed on server. Please contact support or try again.';
        this.processingPayment = false; 
