...
export class CustomerDetailComponent {
    ...
    this.customerService.getCustomerById(this.customerId).subscribe({
      next: (data: CustomerDTO) => {
        ...
        if (!this.customer.profileDetails) {
          this.customer.profileDetails = { firstName: '', lastName: '', phoneNumber: '', addresses: [] };
        }
        if (!this.customer.profileDetails.addresses || this.customer.profileDetails.addresses.length === 0) {
          this.customer.profileDetails.addresses = [{ street: '', city: '', state: '', postalCode: '', country: '', type: 'SHIPPING' }];
        }
        this.loading = false;
      },
      error: (err: HttpErrorResponse) => {
        ...
        this.loading = false;
        console.error('AdminCustomerDetailComponent: Error loading customer:', err);
        if (err.status === 404) {
  ...
  onUpdateCustomer(): void {
    ...
    this.customerService.updateCustomer(this.customerId, this.customer).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        ...
        this.submitting = false;
        console.error('AdminCustomerDetailComponent: Error updating customer:', err);
  ...
  removeAddress(index: number): void {
    if (this.customer?.profileDetails?.addresses && this.customer.profileDetails.addresses.length > 1) {
